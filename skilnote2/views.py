...
from .models import MyShortCutForSkilNote2, TypeForSkilNote2, CategoryForSkilNote2, CategoryNickForSkilNote2, CommentForShortCutForSkilNote2 , TempMyShortCutForSkilNote2, TempMyShortCutForBackEndForSkilNote2, CommentForShortCutForSkilNote2, RecommandationUserAboutSkillNote2, CommentForPageForSkilNote2, GuestBookForSkilNote2
...
class MyShortCutListView2(LoginRequiredMixin,ListView):
    ...
    def get_queryset(self):
        ...
        qs = MyShortCutForSkilNote2.objects.filter(Q(author=user, category = selected_category_id)).order_by('created')
        return qs
    ...
    def get_template_names(self):
        ...
        if self.request.user.is_anonymous:
            return MyShortCutForSkilNote2.objects.filter(author=self.request.user).order_by('created')
        else:
            selected_category_id = self.request.user.profile.selected_category_id
            return MyShortCutForSkilNote2.objects.filter(Q(author=user, category = selected_category_id)).order_by('created')

    def get_context_data(self, *, object_list=None, **kwargs):
        # 카테고리 각각의 정보를 저장하는 테이블 없으면 만든다.
        cn = CategoryNickForSkilNote2.objects.get_or_create(
        ...
        context['category_list'] = CategoryForSkilNote2.objects.all()
        # 카테고리 테이블은 id = 1 => ca1 id = 2 => ca2 이런식이므로 id로 ca1, ca2 객체를 생성 가능
        category = CategoryForSkilNote2.objects.get(id=category_id) # __str__ 설정으로 카테고리 객체 = 카테고리.name 이다
...
class MyShortcutListByCategory2(ListView):
    ...
    def get_queryset(self):
        ...
        return MyShortCutForSkilNote2.objects.filter(category=category, author=user).order_by('created')

    def get_context_data(self, *, object_list=None, **kwargs):
        user = User.objects.get(Q(username = self.request.user))

        context = super(type(self), self).get_context_data(**kwargs)
        context['posts_without_category'] = MyShortCutForSkilNote2.objects.filter(category=None,author=user).count()
        context['category_list'] = CategoryForSkilNote2.objects.all()
        context['posts_without_category'] = MyShortCutForSkilNote2.objects.filter(category=None, author=self.request.user).count()
        ...
        if slug == '_none':
            ...
            category = CategoryForSkilNote2.objects.get(slug=slug)
            context['category'] = category
            context['category_nick'] = CategoryNickForSkilNote2.objects.values_list(slug, flat=True).get(author=user)
...
def delete_guest_book_list(request,id):
    ...
    if request.method == "POST" and request.is_ajax():
        gb = GuestBookForSkilNote2.objects.filter(Q(id=id)).delete()
        print('GuestBookForSkilNote2 delete 성공 id : ' , id);
        return JsonResponse({
...
def insert_for_guest_book(request):
    ...
    guest_book = GuestBookForSkilNote2.objects.create(
...
def delete_comment_for_skilpage(request,id):
    user = request.user
    if request.method == "POST" and request.is_ajax():
        comment = CommentForPageForSkilNote2.objects.filter(Q(id=id)).delete()
        print('TempMyShortCutForBackEndForSkilNote2 delete 성공 id : ' , id);
        return JsonResponse({
            'message': 'comment 삭제 성공',
        })
    else:
...
class MyShortcutListByUser(ListView):
    ...
    def get_queryset(self):
        ...
        if self.request.user.is_anonymous:
            # return MyShortCutForSkilNote2.objects.filter(author=user).order_by('created')
            selected_category_id = category_id
            return MyShortCutForSkilNote2.objects.filter(Q(author=user, category = category_id)).order_by('created')
        else:
            selected_category_id = category_id
            return MyShortCutForSkilNote2.objects.filter(Q(author=user, category = category_id)).order_by('created')
    ...
    def get_context_data(self, *, object_list=None, **kwargs):
            ...
            # category = CategoryForSkilNote2.objects.get(id=user.profile.selected_category_id)
            category = CategoryForSkilNote2.objects.get(id=category_id)
            context['category'] = category
            context['category_num'] = category_id
            context['category_nick'] = CategoryNickForSkilNote2.objects.values_list(category.slug, flat=True).get(author=user)

            context['posts_without_category'] = MyShortCutForSkilNote2.objects.filter(category=None, author=user).count()
            context['page_user'] = user
            context['comment_list_for_page'] = CommentForPageForSkilNote2.objects.filter(user_name=user, category_id = category_id)
            context['star_count_for_user'] = RecommandationUserAboutSkillNote2.objects.filter(user=user.id).count
...
def category_plus_1_for_current_user(request):
    # is this possible?
    # for x in range(i, 98)
    #     CategoryNickForSkilNote2.obejcts.filter(author=request.user).update("ca"+(x+1)=F('ca'+x))
    ...
    CategoryNickForSkilNote2.objects.filter(
        author=request.user
    ).update(**data2)
    ...
    skil_note = MyShortCutForSkilNote2.objects.filter(Q(author=request.user)).order_by("created")

    ca_delete=CategoryForSkilNote2.objects.get(name="ca120")
    MyShortCutForSkilNote2.objects.filter(Q(author=request.user) & Q(category=ca_delete)).delete()
    ...
    for sn in skil_note:
        ...
        if(sn.category.id >= int(ca_num) and sn.category.id != 120):
            ...
            ca = CategoryForSkilNote2.objects.get(id=int(sn.category.id)+1)
            MyShortCutForSkilNote2.objects.filter(id=sn.id).update(category=ca, created=F('created'))
        else:
            print("sn.category.id : ", sn.category.id)
...
def category_minus_1_for_current_user(request):
    # ca=CategoryForSkilNote2.objects.filter(id=category_num)
    ca_num = request.POST['current_ca_num'] # 입력한 ca 번호
    ...
    skil_note = MyShortCutForSkilNote2.objects.filter(Q(author=request.user))

    if(int(ca_num)>1):
        ca_delete_num = int(ca_num)-1

    ca_delete=CategoryForSkilNote2.objects.get(id=ca_delete_num)
    MyShortCutForSkilNote2.objects.filter(Q(author=request.user) & Q(category=ca_delete)).delete()
    # MyShortCutForSkilNote2.obejcts.filter(Q(id=ca))
    ...
    for sn in skil_note:
        ...
        if(sn.category.id >= int(ca_num) and sn.category.id != 1):
            ...
            print("int(sn.category.id)-1 : ", int(sn.category.id)-1)
            ca = CategoryForSkilNote2.objects.get(id=int(sn.category.id)-1)
            MyShortCutForSkilNote2.objects.filter(id=sn.id).update(category=ca, image = F('image'))
...
def move_to_skil_blog(request):
    ...
    if shortcut_ids:
        skill_note_list = MyShortCutForSkilNote2.objects.filter(pk__in=shortcut_ids, author=request.user).order_by('created')
...
def plus_recommand_for_skillnote_user(request):
    ...
    recommand_count = RecommandationUserAboutSkillNote2.objects.filter(Q(user=author) & Q(author_id=me)).count() # 내가 추천한거 있는지 확인
    print("recommand_count : ", recommand_count)

    if (recommand_count ==  0):
        rc = RecommandationUserAboutSkillNote2.objects.create(user=author , author_id=me) # 나의 추천 추가
        print('추천 ++++++++++++++++++++++++++++++++++++++++++++++++++++++')
        recommand_count = RecommandationUserAboutSkillNote2.objects.filter(Q(user=author)).count() # 추천 받은 사람 점수 확인
        ...
        RecommandationUserAboutSkillNote2.objects.filter(Q(user=author) & Q(author_id=me)).delete() # 내가 추천한거 삭제
...
def copy_to_me_from_user_id(request):
    ...
    # 나의 노트 모두 지우기
    if( MyShortCutForSkilNote2.objects.filter(Q(author=request.user)).count() != 0):
        MyShortCutForSkilNote2.objects.filter(Q(author=request.user)).delete()
        CategoryNickForSkilNote2.objects.filter(Q(author=request.user)).delete()
        CommentForShortCutForSkilNote2.objects.filter(Q(author=request.user)).delete()
    ...
    list_for_copy2 = CategoryNickForSkilNote2.objects.filter(Q(author=user_id))
    print("list_for_copy2 : " , list_for_copy2);

    CategoryNickForSkilNote2.objects.filter(Q(author=request.user)).delete()

    for p in list_for_copy2:
        CN = CategoryNickForSkilNote2.objects.create(
            author = request.user,
            ca1 = p.ca1,
            ca2 = p.ca2,
            ca3 = p.ca3,
...
def edit_complete_skill_note_for_backend(request,id):
    ...
    if request.method == "POST" and request.is_ajax():
        ...
        print('TempMyShortCutForBackEndForSkilNote2 text를 ajax로 update textarea')
        print('id : ', id)
        print("content2 : ", content2)
        todo = TempMyShortCutForBackEndForSkilNote2.objects.filter(Q(id=id)).update(content2 = content2)
...
def edit_complete_skill_note_for_front_end(request,id):
    ...
    if request.method == "POST" and request.is_ajax():
        ...
        print('id : ', id)
        print("content2 : ", content2)
        todo = TempMyShortCutForSkilNote2.objects.filter(Q(id=id)).update(content2 = content2)
...
def edit_temp_skill_note_using_textarea_for_backend(request,id):
    ...
    if request.method == "POST" and request.is_ajax():
        ...
        print('TempMyShortCutForSkilNote2 을 ajax로 update textarea')
        print('id : ', id)
        print("content2 : ", content2)
        todo = TempMyShortCutForBackEndForSkilNote2.objects.filter(Q(id=id)).update(content2 = content2)
        print('TempMyShortCutForSkilNote2 update 성공');
...
def edit_temp_skill_note_using_input_for_backend(request,id):
    ...
    if request.method == "POST" and request.is_ajax():
        ...
        print('TempMyShortCutForBackEndForSkilNote2 을 ajax로 update input')
        print('id : ', id)
        print("content1 : ", content1)
        todo = TempMyShortCutForBackEndForSkilNote2.objects.filter(Q(id=id)).update(content1 = content1)
        print('update 성공');
...
def update_temp_skil_title_for_backend(request,id):
    ...
    if request.method == "POST" and request.is_ajax():
        todo = TempMyShortCutForBackEndForSkilNote2.objects.filter(Q(id=id)).update(title=title)
        print('TempMyShortCutForBackEndForSkilNote2 update 성공 id : ' , id);
...
def delete_temp_skill_note_for_backendarea(request,id):
    user = request.user
    if request.method == "POST" and request.is_ajax():
        todo = TempMyShortCutForBackEndForSkilNote2.objects.filter(Q(id=id)).delete()
        print('TempMyShortCutForBackEndForSkilNote2 delete 성공 id : ' , id);
        return JsonResponse({
...
def edit_temp_skill_note_for_input(request,id):
    ...
    if request.method == "POST" and request.is_ajax():
        ...
        print('TempMyShortCutForSkilNote2 을 ajax로 update input')
        print('id : ', id)
        print("content1 : ", content1)
        todo = TempMyShortCutForSkilNote2.objects.filter(Q(id=id)).update(content1 = content1)
        print('update 성공');
...
def delete_temp_memo_by_ajax(request,id):
    user = request.user
    if request.method == "POST" and request.is_ajax():
        todo = TempMyShortCutForSkilNote2.objects.filter(Q(id=id)).delete()
        print('TempMyShortCutForSkilNote2 delete 성공 id : ' , id);
        return JsonResponse({
            'message': 'shortcut 삭제 성공',
        })
    else:
...
def update_temp_skil_title(request,id):
    ...
    if request.method == "POST" and request.is_ajax():
        todo = TempMyShortCutForSkilNote2.objects.filter(Q(id=id)).update(title=title)
        print('TempMyShortCutForSkilNote2 update 성공 id : ' , id);
...
def copyForCategorySubjectToMyCategory(request):
	...
	MyShortCutForSkilNote2.objects.filter(Q(author=request.user) & Q(category=destination_category)).delete()

	user_id = User.objects.get(username=author).id
	ca_id = CategoryForSkilNote2.objects.get(name=original_category)

	list_for_copy = MyShortCutForSkilNote2.objects.filter(Q(author=user_id) & Q(category = ca_id))

	category = CategoryForSkilNote2.objects.get(id=destination_category)

	for p in list_for_copy:
		profile = MyShortCutForSkilNote2.objects.create(
			author = request.user,
			title = p.title,
			content1 = p.content1,
			content2 = p.content2,
...
class search_skil_note_by_word(ListView):
    ...
    def get_queryset(self,request):
        ...
        # 쿼리셋 객체 생성
        qs = MyShortCutForSkilNote2.objects.filter(Q(author = user)).filter(Q(title__icontains=search_word) | Q(content1__icontains=search_word) | Q(content2__icontains=search_word)).order_by('-category')
        return qs
...
def delete_comment_for_my_shortcut(request, shortcut_comment_id):
    ...
    co = CommentForShortCutForSkilNote2.objects.filter(id=shortcut_comment_id).delete()
...
def update_comment_for_my_shortcut(request, shortcut_comment_id):
    print('shortcut_comment_id : ' , shortcut_comment_id)
    co = CommentForShortCutForSkilNote2.objects.filter(id=shortcut_comment_id).update(
        title= request.POST['title'],
...
def new_comment_for_my_shortcut(request, shortcut_id):
    ...
    shortcut = MyShortCutForSkilNote2.objects.get(id=shortcut_id)
    co = CommentForShortCutForSkilNote2.objects.create(
        shortcut= shortcut,
        author = request.user,
        title="default title",
...
def favorite_user_list_for_skillnote(request):
    if request.method == 'GET':
        ...
        my_favorite = []
        ru = RecommandationUserAboutSkillNote2.objects.filter(author_id=request.user)
...
def update_shortcut_nick(request):
    if request.method == "POST" and request.is_ajax():
        ...
        cn = CategoryNickForSkilNote2.objects.filter(id=ca_id).update(**{field: ca_nick_update})
...
def update_shortcut_nick2(request):
    if request.method == "POST" and request.is_ajax():
        ca_id = CategoryNickForSkilNote2.objects.get(author=request.user).id
        field = request.POST['field']
        ca_nick_update = request.POST['ca_nick_update']
...
def update_shortcut1_ajax(request,id):
    ...
    if request.method == "POST" and request.is_ajax():
        ...
        print('id : ', id)
        print("content1 : ", content1)
        todo = MyShortCutForSkilNote2.objects.filter(Q(id=id)).update(content1 = content1)
        print('update 성공');
...
def update_shortcut2_ajax(request,id):
    ...
    if request.method == "POST" and request.is_ajax():
        ...
        print('id : ', id)
        print("content2 : ", content2)
        todo = MyShortCutForSkilNote2.objects.filter(Q(id=id)).update(content2 = content2)
        print('update 성공');
...
def delete_shortcut_ajax(request,id):
    user = request.user
    if request.method == "POST" and request.is_ajax():
        todo = MyShortCutForSkilNote2.objects.filter(Q(id=id)).delete()
        print('MyShortCutForSkilNote2 delete 성공 id : ' , id);
        return JsonResponse({
            'message': 'shortcut 삭제 성공',
        })
    else:
...
def update_shortcut_ajax(request,id):
    ...
    if request.method == "POST" and request.is_ajax():
        todo = MyShortCutForSkilNote2.objects.filter(Q(id=id)).update(title=title)
        print('MyShortCutForSkilNote2 update 성공 id : ' , id);
        return JsonResponse({
            'message': 'shortcut 업데이트 성공',
            'title':title
        })
...
def update_skil_note_file_name(request,id):
    ...
    if request.method == "POST" and request.is_ajax():
        sn = MyShortCutForSkilNote2.objects.filter(Q(id=id)).update(filename=file_name)
...
class SkilNoteListView(LoginRequiredMixin,ListView):
    ...
    def get_context_data(self, *, object_list=None, **kwargs):
        ...
        category_list = CategoryForSkilNote2.objects.all()
        ...
        if not category_list:
            for num in range(1,121):
                ...
                CategoryForSkilNote2.objects.create(pk=num , name=filed_name, slug=slug_name, author=self.request.user)
                print("카테고리 row 생성 성공 ca",num)
                category_list = CategoryForSkilNote2.objects.all()
        else:
            print("카테고리가 이미 존재 ok!!!!!!!!")
            context['category_list'] = category_list
            category = CategoryForSkilNote2.objects.get(id=self.request.user.profile.selected_category_id)
...
class search_skil_note_for_me(LoginRequiredMixin,ListView):
    ...
    def get_queryset(self):
        ...
        qs = MyShortCutForSkilNote2.objects.filter(Q(author=self.request.user) & (Q(title__contains=query) | Q(filename__contains=query) | Q(content1__contains=query) | Q(content2__contains=query))).order_by('created')
        print("qs : ", qs)
        return qs
...
class search_skilnote_by_file_name_for_me(LoginRequiredMixin,ListView):
    ...
    def get_queryset(self):
        ...
        qs = MyShortCutForSkilNote2.objects.filter(Q(author=self.request.user) & Q(filename__contains=query)).order_by('created')
        print("qs : ", qs)
        return qs
...
class search_skilnote_by_file_name_for_all(LoginRequiredMixin,ListView):
    ...
    def get_queryset(self):
        ...
        qs = MyShortCutForSkilNote2.objects.filter(Q(filename__contains=query)).exclude(Q(filename__isnull=True) | Q(filename__exact='')).order_by('created')
        print("qs : ", qs)
        return qs
...
class search_skil_note_for_all(LoginRequiredMixin,ListView):
    ...
    def get_queryset(self):
        ...
        qs = MyShortCutForSkilNote2.objects.filter(Q(title__contains=query) | Q(filename__contains=query) | Q(content1__contains=query) | Q(content2__contains=query)).order_by('created')
        print("qs : ", qs)
        return qs
...
class SkilNoteCreateView_image_through(LoginRequiredMixin,CreateView):
    ...
    def form_valid(self, form):
        ...
        ty = TypeForSkilNote2.objects.get(type_name="image")
        print("ty : ", ty)
        ms = form.save(commit=False)
        ms.author = self.request.user
