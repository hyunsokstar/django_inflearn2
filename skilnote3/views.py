...
from .models import MyShortCutForSkilNote3, TypeForSkilNote3, CategoryForSkilNote3, CategoryNickForSkilNote3, CommentForShortCutForSkilNote3 , TempMyShortCutForSkilNote3, TempMyShortCutForBackEndForSkilNote3, CommentForShortCutForSkilNote3, RecommandationUserAboutSkilNote3, CommentForPageForSkilNote3, GuestBookForSkilNote3
...
def manualPage(request):
    return render(request, 'skilnote3/manual.html', {
	})

def intro_for_skilnote(request):
    return render(request, 'skilnote3/intro_page.html', {
	})


# 입력 모드
class MyShortCutListView2(LoginRequiredMixin,ListView):
    model = MyShortCutForSkilNote3

    def get_queryset(self):
        user = self.request.user
        print("self.request.user : ", self.request.user)
        selected_category_id = user.profile.selected_category_id
        qs = MyShortCutForSkilNote3.objects.filter(Q(author=user, category = selected_category_id)).order_by('created')
        return qs
    ...
    def get_template_names(self):
        if self.request.is_ajax():
            ...
            return ['skilnote3/myshortcut_list2.html']
        return ['skilnote3/myshortcut_list2.html']

        print("user : ", user)

        if self.request.user.is_anonymous:
            return MyShortCutForSkilNote3.objects.filter(author=self.request.user).order_by('created')
        else:
            selected_category_id = self.request.user.profile.selected_category_id
            return MyShortCutForSkilNote3.objects.filter(Q(author=user, category = selected_category_id)).order_by('created')

    def get_context_data(self, *, object_list=None, **kwargs):
        # 카테고리 각각의 정보를 저장하는 테이블 없으면 만든다.
        cn = CategoryNickForSkilNote3.objects.get_or_create(
        ...
        context['category_list'] = CategoryForSkilNote3.objects.all()
        # 카테고리 테이블은 id = 1 => ca1 id = 2 => ca2 이런식이므로 id로 ca1, ca2 객체를 생성 가능
        category = CategoryForSkilNote3.objects.get(id=category_id) # __str__ 설정으로 카테고리 객체 = 카테고리.name 이다
        context['category'] = category
        context['category_id'] = category_id
        # 카테고리 닉은 author = 현재 유저 이름이고 각각의 컬럼에 ca1, ca2, ca3, ca4 등의 카테고리 주제값이 들어있으므로 category.name로 검색해서 가져온다.
        context['category_nick'] = CategoryNickForSkilNote3.objects.values_list(category.name, flat=True).get(author=self.request.user)
...
class MyShortcutListByCategory2(ListView):
    ...
    def get_queryset(self):
        ...
        return MyShortCutForSkilNote3.objects.filter(category=category, author=user).order_by('created')

    def get_context_data(self, *, object_list=None, **kwargs):
        user = User.objects.get(Q(username = self.request.user))

        context = super(type(self), self).get_context_data(**kwargs)
        context['posts_without_category'] = MyShortCutForSkilNote3.objects.filter(category=None,author=user).count()
        context['category_list'] = CategoryForSkilNote3.objects.all()
        context['posts_without_category'] = MyShortCutForSkilNote3.objects.filter(category=None, author=self.request.user).count()
        ...
        if slug == '_none':
            ...
            category = CategoryForSkilNote3.objects.get(slug=slug)
            context['category'] = category
            context['category_nick'] = CategoryNickForSkilNote3.objects.values_list(slug, flat=True).get(author=user)
...
def guest_book_list(request,guest_book_owner):
    if request.method == 'GET':
        ...
        object_list = GuestBookForSkilNote3.objects.filter(owner_for_guest_book=owner).order_by('created_at');
        print("object_list : ", object_list)

        return render(request, 'skilnote3/guest_book_list.html', {
...
def insert_temp_skill_note_for_textarea(request):
    ...
    ty = TypeForSkilNote3.objects.get(type_name="textarea")
    category_id = request.user.profile.selected_category_id
    ca = CategoryForSkilNote3.objects.get(id=category_id)
    title = request.POST['title']

    skilnote3 = TempMyShortCutForSkilNote3.objects.create(
        author = request.user,
        title=title,
        type= ty,
        category = ca,
...
def delete_guest_book_list(request,id):
    ...
    if request.method == "POST" and request.is_ajax():
        gb = GuestBookForSkilNote3.objects.filter(Q(id=id)).delete()
        print('GuestBookForSkilNote3 delete 성공 id : ' , id);
        return JsonResponse({
            'message': 'comment 삭제 성공 ',
        })
    else:
        return redirect('/skilnote3/myshorcut/')
...
def insert_for_guest_book(request):
    ...
    guest_book = GuestBookForSkilNote3.objects.create(
...
def delete_comment_for_skilpage(request,id):
    user = request.user
    if request.method == "POST" and request.is_ajax():
        comment = CommentForPageForSkilNote3.objects.filter(Q(id=id)).delete()
        print('TempMyShortCutForBackEndForSkilNote3 delete 성공 id : ' , id);
        return JsonResponse({
            'message': 'comment 삭제 성공',
        })
    else:
...
def new_comment_for_skilpage(request, user_name, category_id):
    ...
    if request.method == 'POST':
        ...
        if comment_form.is_valid():
            ...
            comment.save()
            return redirect('/skilnote3/myshortcut/'+user_name+"/"+category_id)
    else:
        return redirect('/skilnote3/myshortcut/'+user_name+"/"+category_id)
...
class MyShortcutListByUser(ListView):
    model = MyShortCutForSkilNote3
    paginate_by = 20
    template_name = 'skilnote3/myshortcut_list_for_user.html'
    ...
    def get_queryset(self):
        ...
        if self.request.user.is_anonymous:
            # return MyShortCutForSkilNote3.objects.filter(author=user).order_by('created')
            selected_category_id = category_id
            return MyShortCutForSkilNote3.objects.filter(Q(author=user, category = category_id)).order_by('created')
        else:
            selected_category_id = category_id
            return MyShortCutForSkilNote3.objects.filter(Q(author=user, category = category_id)).order_by('created')
    ...
    def get_context_data(self, *, object_list=None, **kwargs):
            ...
            # category = CategoryForSkilNote3.objects.get(id=user.profile.selected_category_id)
            category = CategoryForSkilNote3.objects.get(id=category_id)
            context['category'] = category
            context['category_num'] = category_id
            context['category_nick'] = CategoryNickForSkilNote3.objects.values_list(category.slug, flat=True).get(author=user)

            context['posts_without_category'] = MyShortCutForSkilNote3.objects.filter(category=None, author=user).count()
            context['page_user'] = user
            context['comment_list_for_page'] = CommentForPageForSkilNote3.objects.filter(user_name=user, category_id = category_id)
            context['star_count_for_user'] = RecommandationUserAboutSkilNote3.objects.filter(user=user.id).count
...
def category_plus_1_for_current_user(request):
    # is this possible?
    # for x in range(i, 98)
    #     CategoryNickForSkilNote3.obejcts.filter(author=request.user).update("ca"+(x+1)=F('ca'+x))
    ...
    CategoryNickForSkilNote3.objects.filter(
        author=request.user
    ).update(**data2)
    ...
    skil_note = MyShortCutForSkilNote3.objects.filter(Q(author=request.user)).order_by("created")

    ca_delete=CategoryForSkilNote3.objects.get(name="ca120")
    MyShortCutForSkilNote3.objects.filter(Q(author=request.user) & Q(category=ca_delete)).delete()
    ...
    for sn in skil_note:
        ...
        if(sn.category.id >= int(ca_num) and sn.category.id != 120):
            ...
            ca = CategoryForSkilNote3.objects.get(id=int(sn.category.id)+1)
            MyShortCutForSkilNote3.objects.filter(id=sn.id).update(category=ca, created=F('created'))
        else:
            print("sn.category.id : ", sn.category.id)
...
def category_minus_1_for_current_user(request):
    # ca=CategoryForSkilNote3.objects.filter(id=category_num)
    ca_num = request.POST['current_ca_num'] # 입력한 ca 번호
    ...
    skil_note = MyShortCutForSkilNote3.objects.filter(Q(author=request.user))

    if(int(ca_num)>1):
        ca_delete_num = int(ca_num)-1

    ca_delete=CategoryForSkilNote3.objects.get(id=ca_delete_num)
    MyShortCutForSkilNote3.objects.filter(Q(author=request.user) & Q(category=ca_delete)).delete()
    # MyShortCutForSkilNote3.obejcts.filter(Q(id=ca))
    ...
    for sn in skil_note:
        ...
        if(sn.category.id >= int(ca_num) and sn.category.id != 1):
            ...
            print("int(sn.category.id)-1 : ", int(sn.category.id)-1)
            ca = CategoryForSkilNote3.objects.get(id=int(sn.category.id)-1)
            MyShortCutForSkilNote3.objects.filter(id=sn.id).update(category=ca, image = F('image'))
...
def move_to_skil_blog(request):
    ...
    if shortcut_ids:
        skill_note_list = MyShortCutForSkilNote3.objects.filter(pk__in=shortcut_ids, author=request.user).order_by('created')
...
def plus_recommand_for_skillnote_user(request):
    ...
    recommand_count = RecommandationUserAboutSkilNote3.objects.filter(Q(user=author) & Q(author_id=me)).count() # 내가 추천한거 있는지 확인
    print("recommand_count : ", recommand_count)

    if (recommand_count ==  0):
        rc = RecommandationUserAboutSkilNote3.objects.create(user=author , author_id=me) # 나의 추천 추가
        print('추천 ++++++++++++++++++++++++++++++++++++++++++++++++++++++')
        recommand_count = RecommandationUserAboutSkilNote3.objects.filter(Q(user=author)).count() # 추천 받은 사람 점수 확인
        ...
        RecommandationUserAboutSkilNote3.objects.filter(Q(user=author) & Q(author_id=me)).delete() # 내가 추천한거 삭제
...
def copy_to_me_from_user_id(request):
    ...
    # 나의 노트 모두 지우기
    if( MyShortCutForSkilNote3.objects.filter(Q(author=request.user)).count() != 0):
        MyShortCutForSkilNote3.objects.filter(Q(author=request.user)).delete()
        CategoryNickForSkilNote3.objects.filter(Q(author=request.user)).delete()
        CommentForShortCutForSkilNote3.objects.filter(Q(author=request.user)).delete()

    user_id = User.objects.get(username=author).id
    print("user_id : " , user_id)

    skilnote3_list_for_copy = MyShortCutForSkilNote3.objects.filter(Q(author=user_id))
    print("skilnote3_list_for_copy : " , skilnote3_list_for_copy);
    MyShortCutForSkilNote3.objects.filter(Q(author=request.user)).delete()

    comment_skilnote3_list_for_copy = CommentForShortCutForSkilNote3.objects.filter(Q(author=user_id))


    for p in skilnote3_list_for_copy:
        myshortcut = MyShortCutForSkilNote3.objects.create(
            author = request.user,
            title = p.title,
            content1 = p.content1,
            content2 = p.content2,
        ...
        for comment in comment_skilnote3_list_for_copy:
            # print("comment.id : ", comment.id)
            # print("myshortcut.id : ", myshortcut.id )
            if comment.shortcut.id == p.id:
                print("댓글 생성 시도 확인")
                skilnote3 = MyShortCutForSkilNote3.objects.filter(id = comment.id)
                skilnote3_comment = CommentForShortCutForSkilNoteForSkilNote3.objects.create(
                    author = request.user,
                    title=comment.title,
                    shortcut = myshortcut,
    ...
    list_for_copy2 = CategoryNickForSkilNote3.objects.filter(Q(author=user_id))
    print("list_for_copy2 : " , list_for_copy2);

    CategoryNickForSkilNote3.objects.filter(Q(author=request.user)).delete()

    for p in list_for_copy2:
        CN = CategoryNickForSkilNote3.objects.create(
            author = request.user,
            ca1 = p.ca1,
            ca2 = p.ca2,
            ca3 = p.ca3,
...
def edit_complete_skill_note_for_backend(request,id):
    ...
    if request.method == "POST" and request.is_ajax():
        ...
        todo = TempMyShortCutForBackEndForSkilNote3.objects.filter(Q(id=id)).update(content2 = content2)
...
def edit_complete_skill_note_for_front_end(request,id):
    ...
    if request.method == "POST" and request.is_ajax():
        ...
        print('id : ', id)
        print("content2 : ", content2)
        todo = TempMyShortCutForSkilNote3.objects.filter(Q(id=id)).update(content2 = content2)
...
def edit_temp_skill_note_using_input_for_backend(request,id):
    ...
    if request.method == "POST" and request.is_ajax():
        ...
        todo = TempMyShortCutForBackEndForSkilNote3.objects.filter(Q(id=id)).update(content1 = content1)
        print('update 성공');
...
def update_temp_skil_title_for_backend(request,id):
    ...
    if request.method == "POST" and request.is_ajax():
        todo = TempMyShortCutForBackEndForSkilNote3.objects.filter(Q(id=id)).update(title=title)
        print('TempMyShortCutForBackEndForSkilNote3 update 성공 id : ' , id);
...
def delete_temp_skill_note_for_backendarea(request,id):
    user = request.user
    if request.method == "POST" and request.is_ajax():
        todo = TempMyShortCutForBackEndForSkilNote3.objects.filter(Q(id=id)).delete()
        print('TempMyShortCutForBackEndForSkilNote3 delete 성공 id : ' , id);
        return JsonResponse({
...
def insert_temp_skill_note_using_input_for_backend(request):
    ...
    ty = TypeForSkilNote3.objects.get(type_name="input")
    category_id = request.user.profile.selected_category_id
    ca = CategoryForSkilNote3.objects.get(id=category_id)
    title = request.POST['title']

    skilnote3 = TempMyShortCutForBackEndForSkilNote3.objects.create(
        author = request.user,
        title=title,
        type= ty,
        category = ca,
    ...
    return JsonResponse({
        ...
        'shortcut_id':skilnote3.id,
        'shortcut_title':skilnote3.title,
        'shortcut_content':skilnote3.content1,
    })
...
def temp_skill_list_for_backend1(request):
    ...
    object_list = TempMyShortCutForBackEndForSkilNote3.objects.filter(author=user)

    return render(request, 'skilnote3/TempMyShortCutForBackEnd_list.html', {
        'object_list': object_list,
        'page_user': user
    })
...
def temp_skill_list1(request):
    ...
    print("user : ", user)
    object_list = TempMyShortCutForSkilNote3.objects.filter(author=user)

    return render(request, 'skilnote3/TempMyShortCut_list.html', {
        'object_list': object_list,
        'page_user': user
    })
...
def edit_temp_skill_note_for_input(request,id):
    ...
    if request.method == "POST" and request.is_ajax():
        ...
        print('id : ', id)
        print("content1 : ", content1)
        todo = TempMyShortCutForSkilNote3.objects.filter(Q(id=id)).update(content1 = content1)
        print('update 성공');
...
def update_temp_skill_note_for_textarea(request,id):
    ...
    if request.method == "POST" and request.is_ajax():
        ...
        print('TempMyShortCutForSkilNote3 을 ajax로 update textarea')
        print('id : ', id)
        print("content2 : ", content2)
        todo = TempMyShortCutForSkilNote3.objects.filter(Q(id=id)).update(content2 = content2)
        print('TempMyShortCutForSkilNote3 update 성공');
...
def delete_temp_memo_by_ajax(request,id):
    user = request.user
    if request.method == "POST" and request.is_ajax():
        todo = TempMyShortCutForSkilNote3.objects.filter(Q(id=id)).delete()
        print('TempMyShortCutForSkilNote3 delete 성공 id : ' , id);
        return JsonResponse({
            'message': 'shortcut 삭제 성공',
        })
    else:
...
def update_temp_skil_title(request,id):
    ...
    if request.method == "POST" and request.is_ajax():
        todo = TempMyShortCutForSkilNote3.objects.filter(Q(id=id)).update(title=title)
        print('TempMyShortCutForSkilNote3 update 성공 id : ' , id);
...
def copyForCategorySubjectToMyCategory(request):
	...
	MyShortCutForSkilNote3.objects.filter(Q(author=request.user) & Q(category=destination_category)).delete()

	user_id = User.objects.get(username=author).id
	ca_id = CategoryForSkilNote3.objects.get(name=original_category)

	list_for_copy = MyShortCutForSkilNote3.objects.filter(Q(author=user_id) & Q(category = ca_id))

	category = CategoryForSkilNote3.objects.get(id=destination_category)

	for p in list_for_copy:
		profile = MyShortCutForSkilNote3.objects.create(
			author = request.user,
			title = p.title,
			content1 = p.content1,
			content2 = p.content2,
...
class search_skil_note_by_word(ListView):
    ...
    def get_queryset(self,request):
        ...
        # 쿼리셋 객체 생성
        qs = MyShortCutForSkilNote3.objects.filter(Q(author = user)).filter(Q(title__icontains=search_word) | Q(content1__icontains=search_word) | Q(content2__icontains=search_word)).order_by('-category')
        return qs
...
class searchSkilNoteViewByIdAndWord(ListView):
    model = MyShortCutForSkilNote3
    paginate_by = 10
    template_name = 'skilnote3/MyShortCut_list_for_search.html'
...
def delete_comment_for_my_shortcut(request, shortcut_comment_id):
    ...
    co = CommentForShortCutForSkilNote3.objects.filter(id=shortcut_comment_id).delete()
...
def update_comment_for_my_shortcut(request, shortcut_comment_id):
    print('shortcut_comment_id : ' , shortcut_comment_id)
    co = CommentForShortCutForSkilNote3.objects.filter(id=shortcut_comment_id).update(
        title= request.POST['title'],
...
def new_comment_for_my_shortcut(request, shortcut_id):
    ...
    shortcut = MyShortCutForSkilNote3.objects.get(id=shortcut_id)
    co = CommentForShortCutForSkilNote3.objects.create(
        shortcut= shortcut,
        author = request.user,
        title="default title",
...
#     # title = request.POST['title']
#
#     skilnote31 = MyShortCutForSkilNote3.objects.create(
#         author = request.user,
#         title="title1",
#         type= ty,
#         category = ca,
#         content2 = ""
#     )
#     skilnote32 = MyShortCutForSkilNote3.objects.create(
#         author = request.user,
#         title="title2",
#         type= ty,
#         category = ca,
#         content2 = ""
#     )
#     skilnote33 = MyShortCutForSkilNote3.objects.create(
#         author = request.user,
#         title="title3",
#         type= ty,
#         category = ca,
#         content2 = ""
#     )
#     skilnote34 = MyShortCutForSkilNote3.objects.create(
#         author = request.user,
#         title="title4",
#         type= ty,
#         category = ca,
#         content2 = ""
#     )
#     skilnote35 = MyShortCutForSkilNote3.objects.create(
#         author = request.user,
#         title="title5",
#         type= ty,
#         category = ca,
...
#     )
#
#     print("skilnote31 : ", skilnote31)
#
#     return JsonResponse({
#         'message': 'textarea 박스 추가 성공',
#         'shortcut_id':skilnote31.id,
#         'shortcut_title':skilnote31.title,
#         'shortcut_content2':skilnote31.content2,
#     })
...
def create_new1_input(request):
    ...
    skilnote3 = MyShortCutForSkilNote3.objects.create(
        author = request.user,
        title=title,
        type= ty,
        category = ca,
...
def create_new1_input_first(request):
    ...
    current_first = MyShortCutForSkilNote3.objects.filter(Q(category=category_id) & Q(author=request.user)).order_by("created").first();
...
def create_new1_input_between(request,current_article_id):
    ...
    current_article = MyShortCutForSkilNote3.objects.get(id=current_article_id)
    print("current_article_time : " , current_article.created)

    smae_category_for_current_article=MyShortCutForSkilNote3.objects.filter(author= current_article.author, category = current_article.category).order_by("created")
    ...
    for i,p in enumerate(smae_category_for_current_article):
        ...
        if (p.created <= current_article.created):
            MyShortCutForSkilNote3.objects.filter(id=p.id).update(created = F('created')+timedelta(seconds=0))
        else:
            MyShortCutForSkilNote3.objects.filter(id=p.id).update(created = F('created')+timedelta(seconds=i+1))
...
def create_new2_textarea_first(request):
    ...
    return JsonResponse({
        ...
        'shortcut_title':skilnote3.title,
        'shortcut_content2':skilnote3.content2,
        # 'author':skilnote3.author.username,
    })
...
def create_summernote_first(request):
    ...
    ty = TypeForSkilNote3.objects.get(type_name="summer_note")
    category_id = request.user.profile.selected_category_id
    ca = CategoryForSkilNote3.objects.get(id=category_id)
    title = request.POST['title']
...
def update_category_by_ajax(request):
    ...
    for i, sn in enumerate(shortcut_ids):
        MyShortCutForSkilNote3.objects.filter(id=sn, author=request.user).update(category=category, created = datetime.now()+timedelta(seconds=i),image=F('image'))

    return redirect('/skilnote3/myshortcut')

def delete_myshortcut_by_ajax(request):
    shortcut_ids = request.POST.getlist('shortcut_arr[]')
    if shortcut_ids:
        MyShortCutForSkilNote3.objects.filter(pk__in=shortcut_ids, author=request.user).delete()
...
def update_my_shortcut_subject(request):
    if request.method == "POST" and request.is_ajax():
        ...
        return redirect('/skilnote3/shortcut')
...
def favorite_user_list_for_skillnote(request):
    if request.method == 'GET':
        ...
        my_favorite = []
        ru = RecommandationUserAboutSkilNote3.objects.filter(author_id=request.user)
        ...
        return render(request, 'skilnote3/favorite_user_list_for_skilnote.html', {
            "object_list" : object_list,
        })
    else:
        return HttpResponse("Request method is not a GET")
...
class user_list_for_memo_view(ListView):
    ...
    def get_template_names(self):
        if self.request.is_ajax():
            print("user list ajax 요청 확인")
            return ['skilnote3/_user_list_for_memo.html']
        return ['skilnote3/user_list_for_memo.html']
...
def update_shortcut_nick(request):
    if request.method == "POST" and request.is_ajax():
        ...
        cn = CategoryNickForSkilNote3.objects.filter(id=ca_id).update(**{field: ca_nick_update})
...
def update_shortcut_nick2(request):
    if request.method == "POST" and request.is_ajax():
        ca_id = CategoryNickForSkilNote3.objects.get(author=request.user).id
        field = request.POST['field']
        ca_nick_update = request.POST['ca_nick_update']
...
def CategoryNickListByUserId(request, user_name):
    if request.method == 'GET':
        ...
        cn_my = CategoryNickForSkilNote3.objects.get(author=user.id)
        print("cn_my : ", cn_my)

        return render(request, 'skilnote3/categorynick_list.html', {
            "category" : cn_my,
        })
    else:
        return HttpResponse("Request method is not a GET")
...
def CategoryNickListByUserId_for_user(request, user_name):
    if request.method == 'GET':
        ...
        return render(request, 'skilnote3/categorynick_list_for_user.html', {
            "category" : cn_my,
            "page_user": user_name
        })
    else:
...
class update_skilnote_by_summernote(UpdateView):
    ...
    def get_template_names(self):
        return ['skilnote3/myshortcut_summernote_form.html']
...
class modify_myshortcut_by_summer_note2(UpdateView):
    ...
    def get_success_url(self):
        return reverse('skilnote3:my_shortcut_list2')
...
def update_shortcut1_ajax(request,id):
    ...
    if request.method == "POST" and request.is_ajax():
        ...
        print('id : ', id)
        print("content1 : ", content1)
        todo = MyShortCutForSkilNote3.objects.filter(Q(id=id)).update(content1 = content1)
        print('update 성공');
...
def update_shortcut2_ajax(request,id):
    ...
    if request.method == "POST" and request.is_ajax():
        ...
        print('id : ', id)
        print("content2 : ", content2)
        todo = MyShortCutForSkilNote3.objects.filter(Q(id=id)).update(content2 = content2)
        print('update 성공');
...
def delete_shortcut_ajax(request,id):
    user = request.user
    if request.method == "POST" and request.is_ajax():
        todo = MyShortCutForSkilNote3.objects.filter(Q(id=id)).delete()
        print('MyShortCutForSkilNote3 delete 성공 id : ' , id);
        return JsonResponse({
            'message': 'shortcut 삭제 성공',
        })
    else:
...
def update_shortcut_ajax(request,id):
    ...
    if request.method == "POST" and request.is_ajax():
        todo = MyShortCutForSkilNote3.objects.filter(Q(id=id)).update(title=title)
        print('MyShortCutForSkilNote3 update 성공 id : ' , id);
        return JsonResponse({
            'message': 'shortcut 업데이트 성공',
            'title':title
        })
...
def update_skil_note_file_name(request,id):
    ...
    if request.method == "POST" and request.is_ajax():
        sn = MyShortCutForSkilNote3.objects.filter(Q(id=id)).update(filename=file_name)
...
class SkilNoteListView(LoginRequiredMixin,ListView):
    ...
    def get_context_data(self, *, object_list=None, **kwargs):
        ...
        category_list = CategoryForSkilNote3.objects.all()
        ...
        if not category_list:
            for num in range(1,121):
                ...
                CategoryForSkilNote3.objects.create(pk=num , name=filed_name, slug=slug_name, author=self.request.user)
                print("카테고리 row 생성 성공 ca",num)
                category_list = CategoryForSkilNote3.objects.all()
        else:
            print("카테고리가 이미 존재 ok!!!!!!!!")
            context['category_list'] = category_list
            category = CategoryForSkilNote3.objects.get(id=self.request.user.profile.selected_category_id)
...
class search_skil_note_for_me(LoginRequiredMixin,ListView):
    ...
    def get_template_names(self):
        return ['skilnote3/search_skil_note_for_me.html']
    ...
    def get_queryset(self):
        ...
        qs = MyShortCutForSkilNote3.objects.filter(Q(author=self.request.user) & (Q(title__contains=query) | Q(filename__contains=query) | Q(content1__contains=query) | Q(content2__contains=query))).order_by('created')
        print("qs : ", qs)
        return qs
...
class search_skilnote_by_file_name_for_me(LoginRequiredMixin,ListView):
    ...
    def get_template_names(self):
        return ['skilnote3/search_skil_note_for_file_name_for_me.html']
    ...
    def get_queryset(self):
        ...
        qs = MyShortCutForSkilNote3.objects.filter(Q(author=self.request.user) & Q(filename__contains=query)).order_by('created')
        print("qs : ", qs)
        return qs
...
class search_skilnote_by_file_name_for_all(LoginRequiredMixin,ListView):
    ...
    def get_template_names(self):
        return ['skilnote3/search_skil_note_for_file_name_for_all.html']
    ...
    def get_queryset(self):
        ...
        qs = MyShortCutForSkilNote3.objects.filter(Q(filename__contains=query)).exclude(Q(filename__isnull=True) | Q(filename__exact='')).order_by('created')
        print("qs : ", qs)
        return qs
...
class search_skil_note_for_all(LoginRequiredMixin,ListView):
    ...
    def get_template_names(self):
        return ['skilnote3/search_skil_note_for_all_user.html']
    ...
    def get_queryset(self):
        ...
        qs = MyShortCutForSkilNote3.objects.filter(Q(title__contains=query) | Q(filename__contains=query) | Q(content1__contains=query) | Q(content2__contains=query)).order_by('created')
        print("qs : ", qs)
        return qs
...
class SkilNoteCreateView_image_through(LoginRequiredMixin,CreateView):
    ...
    def form_valid(self, form):
        ...
        ty = TypeForSkilNote3.objects.get(type_name="image")
        print("ty : ", ty)
        ms = form.save(commit=False)
        ms.author = self.request.user
...
class CreateSkilNoteBySummerNote(LoginRequiredMixin,CreateView):
    ...
    def form_valid(self, form):
        ...
        type_list = TypeForSkilNote3.objects.all()
        if not type_list:
            TypeForSkilNote3.objects.create(type_name="summer_note")
            TypeForSkilNote3.objects.create(type_name="textarea")
            TypeForSkilNote3.objects.create(type_name="input")
            TypeForSkilNote3.objects.create(type_name="image")
            print("타입 생성 성공 !!!!!!!!")
    ...
    def get_success_url(self):
        category_id = self.request.user.profile.selected_category_id
        return reverse('skilnote3:my_shortcut_list')+'#shortcut_{}'.format(category_id)
...
class createSkilNoteForInsertMode(LoginRequiredMixin,CreateView):
    ...
    def get_success_url(self):
        category_id = self.request.user.profile.selected_category_id
        return reverse('skilnote3:my_shortcut_list2')+'#shortcut_{}'.format(category_id)
...
class SkilNoteCreateView_summernote_through2(LoginRequiredMixin,CreateView):
    ...
    def get_success_url(self):
        category_id = self.request.user.profile.selected_category_id
        return reverse('skilnote3:my_shortcut_list2')+'#shortcut_{}'.format(self.object.id)
